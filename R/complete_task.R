...
complete_task <- function(links_tasks, i = 1, DEBUG = FALSE) { 
  ...
  launch_task <- function(links_tasks, time_wait) {
    cat(crayon::red("Error navigating to link. Retrying after", paste0(time_wait, "s\n")))
    Sys.sleep(time_wait)
    remDr$navigate(links_tasks)
  }
  
  tryCatch(remDr$navigate(links_tasks),
           error = function(e) e,
           finally = launch_task(links_tasks, time_wait = 5))
    ...
    while (continue) {
      ...
      # START -------------------------------------------------------------------
        ...
        if (names(present_elements) == "TEXid" & "BTsta" %in% names(present_buttons)) {
          ...
          present_elements[[1]][[1]]$sendKeysToElement(list(as.character(i)))
